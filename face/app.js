...
const takeAttendanceEl = document.getElementById('takeAttendanceEl');
...
function saveImage(dataURL, name) {
    ...
    const fileName = `${name}_${timeString}.png`;

    // Create a folder for the current date if it doesn't exist
    const folderName = `attendance_${dateString}`;
...
function logout() {
    // Prompt the user to confirm if they want to logout
